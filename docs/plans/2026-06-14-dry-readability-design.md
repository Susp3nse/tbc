...
The architecture itself rated **8/10** from both reviewers independently. This is *not* a redesign. It is the small set of mechanical patterns that escaped the shared-factory net the codebase already uses, plus four cheap readability wins. The payoff is **reuse, not line count**: each duplicated shape becomes one shared definition, so a fix or improvement applied there lands in every class that uses it — with **no loss of per-class readability** and no class locked out of diverging when it genuinely needs to.
...
- **Reusable *and* expandable — the real success criterion.** The win is a single shared shape that is the source of truth: fix it once, every class gets the fix. But each shared shape must stay expandable — it takes per-class **DATA** as opts, and exposes an **override escape hatch** so one class can diverge (a different guard, an extra tier) *without* forcing the change onto the others or forking the shared body. A class that fundamentally doesn't fit the shape stays **bespoke** (e.g. druid recovery, mage ManaGem). The shared default covers the common case; the escape hatch covers the exception; bespoke covers the genuine outlier.
...
| F5 | Racial strategy skeleton across ~18 sites → `create_racial_strategy()` (common case only); **10 fit cleanly** — priest×3 + paladin×3 + mage/arcane + shaman/restoration stay bespoke | M | **Medium** | P4 |
...
Option B means: pick canonical key names, rename every class to them, and migrate the saved values so the config doesn't silently reset. This is **higher effort and higher risk than A** — it touches saved settings data — so P3 grows a migration sub-phase and a dedicated migration verification step (§7, §10; verified manually in-game at end of pass, as no Lua harness exists). The payoff is a single uniform key vocabulary that the factory can assume.
...
- **Mechanism:** `migrate_recovery_keys()` runs **at load, before `refresh_settings` builds `cached_settings`**, iterating the rename map: for each `old → new`, if the stored profile has a value for `old` and not for `new`, copy it to `new` and clear `old`.
- **Idempotent by construction — no flag or version stamp.** After a successful run `old` is cleared, so a second run sees `old == nil` and does nothing. The data state *is* the guard; we don't need a stored `recovery_keys_migrated` flag or a version stamp. (There is no rollout to gate — this ships as one unit and is tested once after the whole pass lands; a multi-version stamping scheme would be solving a problem we don't have.)
- **Placement:** the migration must run after the profile store exists but before settings are cached (a dedicated block ordered before `refresh_settings`, or a guarded call early in `core.lua`'s settings init — pinned in P3a). **No setting may be read at load and captured** — the migration writes the store, it does not capture gameplay settings.
- **Safety:** "copy only if `new` is unset" guarantees we never clobber a value already set under the new key, and the `~= nil` (not truthiness) check carries `false` values over correctly.

### D1.3 — New consequences vs Option A (must be honored)
- P3 effort rises **M → L**; it gains sub-phase **P3a′ (migration shim)**.
- **New Gate B requirement:** migration verification — seed a profile with old keys at non-default values, run the migration, confirm new keys hold those values, old keys are cleared, and a second run is a no-op. **There is no Lua test harness today** (`src/sim/` is a TypeScript damage sim and cannot exercise `GetToggle`/`SetToggle`), so this is verified **manually in-game** as part of the end-of-pass verification. A mocked-toggle Lua harness that could automate it is deferred to its own plan.
- Per-class landing stays **atomic** (schema + middleware + the class's slice of the rename map move together). The shim covers all 4 keys and the whole pass lands before testing, so there is no intermediate state in which a renamed key outlives its reader.
...
- **Gate B emphasis:** sim:hunter unaffected — hunter's own `cd_min_ttd` logic (`rotation.lua:294`, an inverted `ttd_ok` boolean) does NOT match the replaced two-line pattern and is left untouched, so the hunter path is a good null check. Build + luacheck across all touched files, and the grep-residue check (expect **5** hits to remain — NOT 0: the predicate def + the 3 Rogue Rupture gates + the Warrior Shout gate, all intentionally left bespoke; see the plan's P1 do-not-touch list).
...
- **P3a — Audit + build the factories (no class wired, no rename yet).**
  ...
  - `core.lua`: add `register_recovery_middleware(opts)` next to `register_trinket_middleware` (same shape: reads `NS.A`, registers via `rotation_registry:register_middleware`). One general implementation each class tunes via **DATA** opts (no per-class branching baked into the body): `prefix` (→ middleware names + `[MW]` log strings); per-class Healthstone `hp_default` (hunter 40, others 35) + Action tier list (hunter `{HSMaster1,2,3}`, warlock has the Fel tier, others 2-tier); healing-potion `hp_default` (hunter 35, others 25) + Actions; and optional mana config — **per-class mana sub-block `priority`** (warlock ManaPotion/DarkRune at `−5`/`−10`, others `0`/`−5`), a **mana-potion Action tier list** (shaman `{Super, Major}`, others `{Super}`), rune Actions, and per-class `pct_default`s. The **3 standard behaviors are the shared default for every class** — skip while `context.is_stealthed`, require `context.in_combat`, and wrap every consumable Action in `:IsExists() and :IsReady(...)` (rune use also honors the `dark_rune_min_hp` floor). Defaulting them uniformly is what removes the accidental drift and means a future fix to any of them lands in all 8 classes at once. **Expandability:** each standard behavior is exposed as an *optional* override opt (default = the standard), so a single class can later opt out or vary one without touching the body or the other classes — the escape hatch, used only when a real divergence appears. Covers **8 classes** (druid's recovery is embedded form-aware middleware → stays bespoke; mage's `ManaGem` stays bespoke). *(Authoritative per-class opt values: plan §A.)*
...
- **P3a′ — Migration shim (lands BEFORE any rename).** Implement `migrate_recovery_keys()` per D1.2: runs before `refresh_settings` caches settings, copies `old → new` only when `new` is unset (`~= nil` check, not truthiness), clears `old`; idempotent by construction (no flag/version stamp). **Verify per D1.3 — manually in-game** (no Lua harness exists; see §5/§10): seed a profile with old keys at non-default values → `/reload` → confirm new keys hold them, old keys cleared, and a second `/reload` changes nothing. This sub-phase lands before P3b so the shim exists when the first class's reader switches names. Gate B = build + luacheck + manual migration verified.
- **P3b — Migrate classes 1–5 (schema + middleware + rename atomically per class).** Order: hunter, mage, warlock, paladin, warrior. For each class, in one step: swap inline recovery schema for `S.recovery(...)`/`S.mana_recovery(...)` emitting canonical keys; swap hand-rolled recovery middleware for `NS.register_recovery_middleware(...)` reading canonical keys; ensure the class's slice of the rename map is covered by the shim. Hunter additionally picks up the deliberate normalizations (now in-combat/stealth/`IsExists` gated + a `dark_rune_min_hp` floor) — *expected*, not regressions. Build + luacheck + in-game smoke per class (`/reload`, confirm `Build:`/`NS.BUILD_NUMBER` advanced). Checkpoint after class 5.
...
- **Files:** `core.lua` (add factory) + the **10** clean-fit playstyle files (mage×2 [fire, frost], rogue×3, warlock×3, shaman×2 [elemental, enhancement]).
...
- **LEAVE the outliers bespoke (audit-confirmed):** **paladin×3** (HP-gated naaru/stoneform), **priest×3** (no TTD gate, `is_spell_available` guards, inline combat gate; smite diverges further), **mage/arcane** (`if not state.is_burning` burn-phase gate, `arcane.lua:188`), and **shaman/restoration** (omits `is_burst`, `restoration.lua:359` — the factory's hardcoded `is_burst=true` would auto-burst-gate it). Do **not** add `gate`/`condition`/`is_burst`/availability params to force them through — that's the over-abstraction trap.
- **Gate A is decisive here:** audit done — **10 of ~18 sites (~56%) fit cleanly**, clearing the ">half fit" bar, so P4 proceeds with priest + paladin + mage/arcane + shaman/restoration excluded. (If a re-read shrinks this below ~half, descope P4.) This remains an explicit "maybe we don't do this" phase.
...
- **Window fit:** audit + the 10 common-case migrations fit one window.
...
| Recovery key rename silently resets saved settings (Decision D1-B) | Migration shim lands in P3a′ **before** any rename; idempotent by construction, copy-only-if-unset (`~= nil`); **manual in-game migration verification** (no Lua harness) + full-profile smoke at end of pass; per-class atomic schema+middleware+rename; log-string + name equivalence proof. |
...
| Racial strategy spread | ~18 sites; **10 fit cleanly** (mage×2 [fire,frost], rogue×3, warlock×3, shaman×2 [elemental,enhancement]); priest×3 + paladin×3 + mage/arcane (`is_burning` gate) + shaman/restoration (no `is_burst`) bespoke |
