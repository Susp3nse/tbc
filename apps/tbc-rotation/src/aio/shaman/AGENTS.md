...
**`EarthShockR1` = `8042`** is a deliberate separate action — R1 Earth Shock is used *interrupt-only* to save mana (don't replace it with max-rank).
...
Middleware (priority high→low): Interrupt (500, `FORM_RESHIFT` — TBC's *only* shaman interrupt, so it's top) → CurePoison (350) → CureDisease (340) → Healthstone/HealingPotion (RECOVERY_ITEMS 300/295) → ManaPotion (280) → DarkRune (275) → AutoTremor (260) → ShieldMaintain (250) → Purge (200) → WeaponImbues (`SELF_BUFF_OOC` 140).
