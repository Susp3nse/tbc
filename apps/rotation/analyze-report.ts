...
async function main() {
  ...
  if (!reportCode || !Number.isInteger(fightId)) {
    console.error(`
Usage:
  corepack pnpm --filter @flux/rotation analyze:report -- --report <code> --fight <id> [--player <name>] [--class Druid] [--spec Feral]

Examples:
  corepack pnpm --filter @flux/rotation analyze:report -- --report Mz94KqCY8X7LkP23 --fight 31 --player Chancity --class Druid --spec Cat
  corepack pnpm --filter @flux/rotation analyze:report -- --report Mz94KqCY8X7LkP23 --fight 31 --player Chancity --class Hunter
`);
    process.exit(1);
  }
